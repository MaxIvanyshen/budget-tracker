...
func (s *Service) createIncome(ctx context.Context, userID int64, form url.Values) (*sqlc.Transactions, error) {
	amountStr := form.Get("amount")
	amount, err := strconv.ParseInt(amountStr, 10, 64)
	category := form.Get("source")
	...
	s.logger.LogAttrs(ctx, slog.LevelInfo, "Creating income", slog.Any("amount", amount), slog.Any("category", category), slog.Any("description", description), slog.Any("date", dateStr))
